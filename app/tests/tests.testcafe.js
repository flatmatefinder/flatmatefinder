...
import { foryouadminPage } from './foryouadmin.page';
import { mapPage } from './map.page';
import { footer } from './footer.component';
import { suggestionsadminPage } from './suggestionsadmin.page';
...
test('Test that the profile page works', async (testController) => {
  ...
  await profilePage.formCheck(testController);
});

test('Test that the map page works', async (testController) => {
  await navBar.gotoSignInPage(testController);
  await signinPage.signin(testController, credentials.username, credentials.password);
...
test('Test that admin suggestions page works', async (testController) => {
  await navBar.gotoSignInPage(testController);
  await signinPage.signin(testController, AdminCredentials.username, AdminCredentials.password);
  await navBar.gotoSuggestionsAdminPage(testController);
  await suggestionsadminPage.isDisplayed(testController);
});

test('Test that the for you admin suggestions page works', async (testController) => {
  await navBar.gotoSignInPage(testController);
  await signinPage.signin(testController, AdminCredentials.username, AdminCredentials.password);
  await navBar.gotoForYouAdminPage(testController);
  await foryouadminPage.isDisplayed(testController);
});
